...
def CreateLiquidityPool(appID, pairID, depositCoins):
    command = f"fury tx liquidityV1 create-pool {appID} {pairID} {depositCoins} --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --gas 5000000 --keyring-backend test -y"
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
...
def AddAssetRates(assetName, jsonData):
    ...
    command = f"""fury tx gov submit-proposal add-asset-rates-stats --add-asset-rates-stats-file '{fileName}' --from {GENESIS_ACCOUNT_NAME} --chain-id {CHAIN_ID} --keyring-backend test --gas 5000000 -y"""
    output = subprocess.getstatusoutput(command)[1]
    output = json.loads(output)
    if int(output["code"]) != 0:
        print(output)
